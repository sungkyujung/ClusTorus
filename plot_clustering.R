...
b <- data.frame(ia$phi,ia$psi, ia$Chat_mix == 1, ia$Chat_max == 1, ia$Chat_e == 1, ia$Chat_kmeans == 1)
colnames(b) <- c("phi","psi","C_mix","C_max","C_e", "C_kmeans")
  b<- b %>%  pivot_longer(3:6, names_to = "Type", values_to = "value") %>% filter(Type == "C_e")
  g0 <- ggplot() +
    ...
    geom_point(mapping = aes(x,y,shape = c, color = c), data = data.frame(x = data[,1],y =data[,2],
                                                               c = as.factor(c$mixture$cluster.id.by.ehat)))
