...
data16 %>% ggplot(aes(phi, psi)) + geom_point(size = 1) +
...
ggsave("./coronavirus/RamPlot16.png", width = 6, height = 4)

data15 <- torsion_to_dat(pdb.6M15) %>% filter(type == "B") %>% select(phi,psi, position) 
nrow(data15)

alldata <- rbind(data.frame(data16,Source ="SADS-Cov"),
      data.frame(data15,Source ="HKU2"),
      data.frame(dataXX,Source ="SARS-Cov2"))
alldata %>% filter(Source != "HKU2") %>% ggplot(aes(phi, psi,color = Source, shape = Source)) + geom_point() + 
  scale_x_continuous(breaks = c(-2,-1,0,1,2)*90, labels = c("-pi","-pi/2","0","pi/2","pi"))+ 
  scale_y_continuous(breaks = c(-2,-1,0,1,2)*90, labels = c("-pi","-pi/2","0","pi/2","pi")) 
ggsave("./coronavirus/RamPlot_16vsXX.png", width = 6, height = 4)

alldata %>%   ggplot(aes(phi, psi,color = Source, shape = Source)) + geom_point(size = 1) + 
  scale_x_continuous(breaks = c(-2,-1,0,1,2)*90, labels = c("-pi","-pi/2","0","pi/2","pi"))+ 
  scale_y_continuous(breaks = c(-2,-1,0,1,2)*90, labels = c("-pi","-pi/2","0","pi/2","pi")) 
ggsave("./coronavirus/RamPlot_ALL.png", width = 6, height = 4)

data16 %>% ggplot(aes(phi, psi)) + geom_point(size = 1) +
  scale_x_continuous(breaks = c(-2,-1,0,1,2)*90, labels = c("-pi","-pi/2","0","pi/2","pi"))+ 
  scale_y_continuous(breaks = c(-2,-1,0,1,2)*90, labels = c("-pi","-pi/2","0","pi/2","pi")) + 
  geom_point(aes(phi,psi), data = data15, color = "blue",size = 0.5)
ggsave("./coronavirus/RamPlot.png", width = 6, height = 4)
...
g_cp
ggsave(paste(filename,"4cp.png",sep = ""), width = 6, height = 4)
...
#   ggtitle(paste("vM2 Mixture fit with J =",J,"components")) 
# ggsave(paste(filename,"5mixture_fit1.png",sep = ""), width = 6, height = 4)
# 
# temp %>% ggplot(aes(iteration,Delta)) +
#   geom_point() + geom_line() +
#   scale_y_log10() + 
#   ggtitle(paste("vM2 Mixture fit with J =",J,"components")) 
# ggsave(paste(filename,"5mixture_fit2.png",sep = ""), width = 6, height = 4)
# 
# 
# ia <- icp.torus.eval(icp.torus, level = 0.1, eval.point = grid.torus())
# b <- cbind( cp.torus.obj$cp.torus, ia$Chat_mix == 1, ia$Chat_max == 1, ia$Chat_e == 1)
...
#   scale_y_continuous(breaks = c(0,1,2,3,4)*pi/2, labels = c("0","pi/2","pi","3pi/2","2pi"), limits = c(0,2*pi))
# g0 
# ggsave(paste(filename,"6icp.png",sep = ""), width = 6, height = 4)
...
# save.image(file = "coronavirus_intermediate.RData")


# Load Intermediate Data --------------------------------------------------

 
load("coronavirus_intermediate.RData")
...
ggsave(paste(filename,"7icp_criterion_e.png",sep = ""), width = 6, height = 4)
...
temp_out<- out %>% group_by(J) %>% summarise(AUC = mean(1-mu)/2) %>% arrange(desc(AUC)) 
temp_out
Jauc <- c(temp_out$J[1])
...
out %>%  ggplot(aes(alpha,1-mu,color = J, group = J)) + geom_line() +
  geom_point() + scale_colour_distiller(palette="Spectral") +geom_abline(slope = 1, intercept = 1-out[out.index,4]) +
  geom_line(aes(alpha,1-mu),data = out %>% filter(J == Jauc), color = "black", size = 2)
...
out %>% filter(J == Jauc) %>% ggplot(aes(alpha,mu)) + 
  geom_point() + geom_abline(slope = -1, intercept = out[out.index,4])


# inspect a little further on out 
out %>% as.data.frame()  %>% arrange(criterion) %>% head()
best.option <- out %>% as.data.frame()  %>% arrange(criterion) %>% head(1) 
best2.option <- out %>% as.data.frame()  %>% filter(J != 12) %>% arrange(criterion) %>% head(1) 
best.option
best2.option
...
testing <- on.torus(testing) 

rbind( data.frame(phi = testing[,1], psi = testing[,2], Source = "HKU2"), 
       data.frame(phi = data[,1], psi = data[,2], Source = "SADS-CoV")) %>% 
  ggplot(aes(phi,psi, color = Source, shape = Source) ) + geom_point(size = 1)
ggsave("./coronavirus/RamPlot0.png", width = 6, height = 4)
...
Out.dt <- rbind(Out.dt, 
                data.frame(alpha = alphavec, coverage = (1 - alphavec) - 2*sqrt(alphavec*(1-alphavec)/testing.n), Type = "CI(95%)"))
 
head(Out.dt)
Out.dt %>% ggplot(aes(1-alpha, coverage,color = Type, group = Type))  + geom_line() +
  geom_point() + geom_abline(slope = 1, intercept = 0) + geom_vline(xintercept =  1- alphahat,linetype = "dotted" )

Out.dt %>% filter(Type != "CI(95%)") %>% ggplot(aes(1-alpha, coverage,color = Type, group = Type))  +  geom_line(aes(1-alpha, coverage), data = Out.dt %>% filter(Type == "CI(95%)"), color = "gray", size = 2) + geom_line() +
  geom_point() + geom_abline(slope = 1, intercept = 0) + geom_vline(xintercept =  1- alphahat,linetype = "dotted" )


ggsave(paste(filename,"Empirical_coverage.png",sep=""), width = 6, height = 4)
...
Out.dt3 <- data.frame(Out.dt, Data = "SARS-Cov2")
...
Out.dt4 <- data.frame(Out.dt, Data = "SADS-Cov")
...
Out_all  %>% filter(Type != "CI(95%)") %>% ggplot(aes(1-alpha, coverage,color = Type, group = Type))  +  facet_grid(Data ~ . ) +  
  geom_line(aes(1-alpha, coverage), data = Out.dt %>% filter(Type == "CI(95%)"), color = "gray", size = 2) +
  geom_line() + geom_point() + geom_abline(slope = 1, intercept = 0) + geom_vline(xintercept =  1- alphahat,linetype = "dotted" ) 
  



 
ggsave(paste(filename,"Empirical_coverage2.png",sep=""), width = 6, height = 4)
